...
public class BaseEnemyEngine : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        {
            if (transform.position.x >= (border / 2 + startpos.x))
            {
                direction = false;
            }
            else if (transform.position.x <= (-border / 2 + startpos.x))
            {
                direction = true;
            }
        }
    ...
    private void OnCollisionEnter2D(Collision2D collision)
    {
        ...
        else if (life <= 0 && !isBase)
        {
            ...
            source.PlayOneShot(cough);
            //create base enemy
            BaseEnemyEngine newBase = Instantiate(baseEnemy, startpos, transform.localRotation).GetComponent<BaseEnemyEngine>();
